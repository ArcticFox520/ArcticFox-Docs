...
::: center
## 功能图示
![AstrBot-Shell](/Public/image/ASTUI.png)
:::
...
> 1. 获取安装脚本
```bash
curl -o install https://raw.githubusercontent.com/ArcticFox520/AstrBot-Shell/refs/heads/main/install
```
  > 2. 执行安装脚本
```bash
bash install
...
@tab 使用 `git`
  > 1. 克隆仓库
```bash
git clone --depth=1 https://github.com/ArcticFox520/AstrBot-Shell
```
  > 2. 执行脚本
```
bash AstrBot-Shell/run
```
  > 3. 阅读协议
```bash
AstrBot Shell 使用协议
...
@tab 使用 `git`
  > 1. 克隆仓库
```bash
git clone --depth=1 https://gh.arcticfox.top/https://github.com/ArcticFox520/AstrBot-Shell
```
  > 2. 执行脚本
```
bash AstrBot-Shell/run
