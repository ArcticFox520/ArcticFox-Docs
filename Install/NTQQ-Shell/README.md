...
::: center
## 功能图示
![NTQQ-Shell](/Public/image/NSTUI.png)
:::
...
> 1. 获取安装脚本
```bash
curl -o install https://raw.githubusercontent.com/ArcticFox520/NTQQ-Shell/refs/heads/main/install
```
  > 2. 执行安装脚本
```bash
bash install
...
```bash
NTQQ Shell 使用协议
1.欢迎使用NTQQ Shell 项目（以下简称“本项目”）。在使用本项目之前，请仔细阅读并遵守本协议的所有条款和条件。使用本项目即表示您同意受到本协议的约束。
...
最后，感谢您对 NTQQ Shell 项目的信任与支持!
```
  > 4. 开始安装脚本
@tab 使用 `git`
  > 1. 克隆仓库
```bash
git clone --depth=1 https://github.com/ArcticFox520/NTQQ-Shell
```
  > 2. 执行脚本
```
bash NTQQ-Shell/run
...
> 1. 获取安装脚本
```bash
curl -o install https://gh.arcticfox.top/https://raw.githubusercontent.com/ArcticFox520/NTQQ-Shell/refs/heads/main/install
```
  > 2. 执行安装脚本
```bash
bash install
...
@tab 使用 `git`
  > 1. 克隆仓库
```bash
git clone --depth=1 https://gh.arcticfox.top/https://github.com/ArcticFox520/NTQQ-Shell
```
  > 2. 执行脚本
```
bash NTQQ-Shell/run
